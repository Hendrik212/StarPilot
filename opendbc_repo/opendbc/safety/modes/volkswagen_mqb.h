...
static bool volkswagen_mqb_tx_hook(const CANPacket_t *msg) {
  ...
  const TorqueSteeringLimits VOLKSWAGEN_MQB_STEERING_LIMITS = {
    ...
    .max_rt_delta = 188,           // VW: raised ceiling for CRAFTER_MK2's faster rate ramp; 10 max rate up * 50Hz * 250000 RT interval / 1000000 = 125 ; 125 * 1.5 safety pad = 188
    .max_rate_up = 10,             // VW: raised ceiling to allow CRAFTER_MK2's STEER_DELTA_UP=10 (5.0 Nm/s); other MQB cars still request 4 in values.py, so unaffected in practice
